...
RC-PACKAGE was accepted by the controller on 2026-09-14 for `1.0.0-rc.1` at package candidate `c3c9709` (verify `34849865998`: JS 62/62, TS 620/620). Evidence covers `brand:check`, the M7 audit, `tizen:build` with numeric widget version `1.0.0`, 19/19 staged and packaged artifact checks, a local `samsung`-profile signed WGT that was not distributed, and a local 76/76 browser matrix at the candidate version. PR #137 merged with a merge commit. Evidence: `docs/verification/rc-package-execution.md`.
