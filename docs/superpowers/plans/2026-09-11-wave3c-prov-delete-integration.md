...
> **Historical record, imported 2026-09-15.** This document lived only on branch `docs/wave3c-closure` and never reached `main` at the time. The work it describes is integrated on `main`, and its status lines are no longer current. Current status: [`v1-rc-readiness.md`](../../verification/v1-rc-readiness.md).
